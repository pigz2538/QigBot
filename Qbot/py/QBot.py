...
current_key_index = 0
is_group = 1
key_number = len(config_data['openai']['api_key'])
...
def get_message():
    ...
    if request.get_json().get('message_type') == 'group' and is_group:  # 如果是群消息
...
def chat_with_gpt(messages):
    ...
    try:
        ...
        if str(e).__contains__("Rate limit reached for default-gpt-3.5-turbo"):
            # 切换key
            if current_key_index == key_number - 1:
                current_key_index = 0
            else:
